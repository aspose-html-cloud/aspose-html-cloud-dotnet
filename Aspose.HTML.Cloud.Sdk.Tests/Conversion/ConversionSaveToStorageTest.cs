...
namespace Aspose.HTML.Cloud.Sdk.Tests.Conversion
{
    ...
    public class ConversionSaveToStorageTest : BaseTestContext
    {
        ...
        [TestMethod]
        public void Test_PutHtmlConvert_Doc_StorageDocToStorage()
        {
            string name = "testpage1.html";
            string folder = StorageTestDataPath;
            string outPath = Path.Combine(testoutStorageFolder, $"{name}_converted_at_{DateTime.Now.ToString("yyMMdd_hhmmss")}.docx");
            var response = this.HtmlApi.PutConvertDocumentToDoc(name, outPath, null, null, null, null, null, null, folder);
            Assert.IsNotNull(response);
            Assert.AreEqual(200, response.Code);
            Assert.IsTrue(StorageApi.FileOrFolderExists(outPath));
        }
        ...
        [TestMethod]
        public void Test_PostHtmlConvert_Doc_LocalFileToStorage()
        {
            string name = "testpage1.html";
            string outPath = Path.Combine(testoutStorageFolder, $"{name}_converted_at_{DateTime.Now.ToString("yyMMdd_hhmmss")}.docx");
            string srcPath = Path.Combine(LocalTestDataPath, name);
            using (Stream stream = new FileStream(srcPath, FileMode.Open, FileAccess.Read))
            {
                var response = this.HtmlApi.PostConvertDocumentToDoc(stream, outPath);
                Assert.IsNotNull(response);
                Assert.AreEqual(200, response.Code);
            }
            Assert.IsTrue(StorageApi.FileOrFolderExists(outPath));
