...
namespace Aspose.HTML.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeHTMLCloudContext.atcc = new AsposeHTMLCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeHTMLCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeHTMLCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
