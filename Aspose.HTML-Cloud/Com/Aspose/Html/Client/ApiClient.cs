...
namespace Com.Aspose.Html.Client
{
    ...
    public class ApiClient
    {
        ...
        public ApiClient(String apiKey, String apiSid, String basePath= "http://api.aspose.cloud/v1.1")
        {
            BasePath = basePath;
        ...
        private bool processContentAsBinaryFormat(string contentType)
        {
            if (contentType == "application/zip") return true;
            if (s_mimeTypes.Values.Contains<string>(contentType)) return true;
            return false;
        }
        ...
        public object Deserialize(IRestResponse response, Type type)
        {
            ...
            if (type == typeof(Stream))
            {
                ...
                if (headers != null)
                {
                    ...
                    foreach (Parameter hdr in headers)
                    {
                        if (hdr.Type == ParameterType.HttpHeader)
                        {
                            string val = hdr.Value as string;
                            switch (hdr.Name)
                            {
                                case "Content-Disposition":
                                    var match = regex.Match(val);
                                    if (match.Success)
                                        fileName = filePath + match.Value.Replace("filename=", "").Replace("\"", "").Replace("'", "");
                                    break;
                                case "Content-Type":
                                    isBin = processContentAsBinaryFormat(val);
                                    break;
                            }
                        }
                    }
                ...
                {
                    using (FileStream fstr = new FileStream(fileName, FileMode.Create, FileAccess.Write))
                    {
                        byte[] buffer = response.RawBytes;
                        fstr.Write(buffer, 0, buffer.Length);
                        fstr.Flush();
                    }
                }
